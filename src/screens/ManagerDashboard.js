...
export default function ManagerDashboard() {
  ...
  const toggleCreds = async (userId) => {
    ...
    try {
      await updateDoc(doc(db, 'users', userId), {
        canUpdateCredentials: !user.canUpdateCredentials
      });
    } catch (e) {
      console.error(e);
  ...
  const handleRoleChange = (userId) => {
    ...
    if (!user || user.role?.toLowerCase() === 'admin') {
      showAlert('Restricted', 'You cannot change an Admin role.');
      return;
    }

    showRoleSelector(['user', 'manager'], async (nextRole) => {
      setSavingId(userId);
      try {
  ...
  const toggleAll = async (userId, screenKey) => {
    ...
    try {
      ...
      const newScreenPerms = Object.fromEntries(
        PERMISSIONS.map(p => [p, !allOn])
      );
  ...
  const handleDeleteUser = useCallback((userId, role) => {
    ...
    if (r === 'admin' || r === 'manager') {
      showAlert('Restricted', 'You do not have permission to delete this account.');
      return;
    }
    ...
    showConfirm(
      ...
      async () => {
        try {
          ...
          console.error(error);
          // Fallback: Firestore profile only
          try {
    ...
    <ScrollView style={styles.dashContainer} contentContainerStyle={{ paddingBottom: 40, alignItems: 'center' }}>
      ...
      <View style={styles.dashSearchWrapper}>
        <View style={{ width: '100%', marginBottom: 15 }}>
          <Text style={{ fontSize: 24, fontWeight: '900', color: '#1C1C1E', letterSpacing: 0.5 }}>
          ...
          <View style={styles.dashTable}>
            ...
            {filteredUsers.length === 0 ? (
              ...
              filteredUsers.map((u, index) => {
                ...
                const roleClean = (u.role || '').toLowerCase();
                const isAdmin = roleClean === 'admin';
                const isManager = roleClean === 'manager';
                const deleteRestricted = isAdmin || isManager;
                ...
                return (
                  ...
                  >
                    ...
                    <TouchableOpacity
                      ...
                      activeOpacity={deleteRestricted ? 1 : 0.7}
                    >
                      <Ionicons
                        name="trash"
                        size={24}
                        color={deleteRestricted ? '#E5E5EA' : '#FF3B30'}
                      />
                    </TouchableOpacity>
                  </Animated.View>
                );
